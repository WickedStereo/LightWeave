...
| Actual result and evidence | Important backup hashes were identical before and after installation. App Lab compiled/flashed the clone in about 93 seconds; the sketch used 86,468 bytes of flash and 35,070 bytes of global RAM. A 124-byte image and a 188-byte one-second audio payload were acknowledged byte-for-byte, launched with 994/1,506 optical bits, and produced serial start/completion plus laser-off evidence at 24.85/37.65-second estimates. An immediate second request was rejected busy. The browser then generated and sent a 104-byte image through the same dashboard API, displayed the 20.85-second launch evidence, and produced no console errors. The original app remained stopped and unmodified. |
