...
| Windows-to-UNO Q transmitter flow | Standard and parallel targets pass | Commit `028f9d9` provides the original dashboard/ADB flow; the same `lightweave.exe` now defaults to the parallel clone, while constrained `LIGHTWEAVE_UNO_Q_TRANSMITTER_APP=standard` selects the preserved original app. Live parallel status reports ready on transmitter `123900964` |
...
| D-059 | 2026-08-07 | Reuse the existing Windows `lightweave.exe` and add a constrained standard/parallel transmitter-app selector. | The dashboard, codecs, and payload APIs remain one application; the selector changes only the validated ADB app path/name/manifest and rejects arbitrary targets. The owner subsequently selected `parallel` as the laptop default; `LIGHTWEAVE_UNO_Q_TRANSMITTER_APP=standard` remains the explicit rollback. |
| D-060 | 2026-08-07 | Show both one-channel and three-channel optical time estimates in the transmit dashboard. | The existing displayed time described the preserved single-laser waveform and was misleading for the parallel default. The three-channel estimate uses `ceil((payload bytes + 12) / 3)` byte slots, eight bit periods per slot, and the shared start/stop bits; computation latency remains separately labeled. |
...
| 2026-08-07 | Added a constrained parallel-app selector to the existing Windows executable; live dashboard status against transmitter `123900964` returned connected/ready/running with `app_variant: parallel`. |
| 2026-08-07 | Made the parallel transmitter the Windows dashboard default at the owner's request, restarted its already-default App Lab service, and verified connected/ready/running status without the selector environment variable; the original single-laser target remains available through explicit `standard`. |
| 2026-08-07 | Relabeled dashboard optical timing to show one-channel and three-channel estimates together for generated text, image, audio, send confirmation, and launch evidence; removed the misleading single-channel countdown from the generic busy badge. |
