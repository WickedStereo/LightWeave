...
$trackedRoot = (Resolve-Path (Join-Path $projectRoot "uno_q\parallel_transmitter_app")).Path
...
$sourceApp = "/home/arduino/ArduinoApps/lightweave_transmitter"
$targetApp = "/home/arduino/ArduinoApps/lightweave_parallel_transmitter"
$targetDisplayName = "LightWeave Parallel Transmitter"
$requiredFiles = @(
    "app.yaml",
    "README.md",
    "parallel_transmitter.manifest.json",
    "sketch/sketch.ino",
    "sketch/sketch.yaml"
)
...
foreach ($relative in $requiredFiles) {
    ...
    if (-not (Test-Path -LiteralPath $local -PathType Leaf)) {
        throw "Tracked parallel-transmitter file is missing: $relative"
    }
}
...
if (-not $DeviceSerial) { $DeviceSerial = $env:LIGHTWEAVE_UNO_Q_SERIAL }
if (-not $DeviceSerial) {
    $authorized = @(
        (& $AdbPath devices) |
...
if ($LASTEXITCODE -ne 0) { throw "Could not inspect the transmitter UNO Q." }
...
$sourceFiles = @(
    "$sourceApp/app.yaml",
    "$sourceApp/transmitter.manifest.json",
    "$sourceApp/python/main.py",
    "$sourceApp/python/lightweave_transmitter.py",
    "$sourceApp/sketch/sketch.ino"
)
foreach ($remote in $sourceFiles) {
    & $AdbPath -s $DeviceSerial shell "test -f '$remote'"
    if ($LASTEXITCODE -ne 0) {
        throw "The standard transmitter must be installed first: $remote"
    }
}
...
if ($LASTEXITCODE -ne 0 -or -not $sourceBefore) {
    throw "Could not hash the standard transmitter."
}

& $AdbPath -s $DeviceSerial shell "test -d '$targetApp'"
$targetExists = $LASTEXITCODE -eq 0
if ($targetExists) {
    & $AdbPath -s $DeviceSerial shell "test -f '$targetApp/parallel_transmitter.manifest.json'"
    if ($LASTEXITCODE -ne 0) {
        throw "Refusing to overwrite an unrelated lightweave_parallel_transmitter app."
    }
}

Write-Host "Device: $DeviceSerial"
Write-Host "Source app: lightweave_transmitter (read-only)"
Write-Host "Target app: lightweave_parallel_transmitter / $targetDisplayName"
Write-Host "Sketch lanes: D5, D7, D9"
if ($DryRun) {
...
if (-not $targetExists) {
    & $AdbPath -s $DeviceSerial shell "arduino-app-cli app new lightweave_parallel_transmitter --from-app '$sourceApp'"
    if ($LASTEXITCODE -ne 0) { throw "Could not clone the standard transmitter." }
}

$stage = "/tmp/lightweave-parallel-tx-$([guid]::NewGuid().ToString('N').Substring(0, 10))"
...
install -m 0644 '$stage/source/parallel_transmitter.manifest.json' '$targetApp/parallel_transmitter.manifest.json'
...
if ($LASTEXITCODE -ne 0) { throw "Could not install the parallel transmitter overlay." }
...
if ($targetRunning) {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Could not stop the parallel transmitter for its update." }
}
& $AdbPath -s $DeviceSerial shell "arduino-app-cli app clean-cache '$targetApp'"
if ($LASTEXITCODE -ne 0) { throw "Could not clear the parallel transmitter cache." }

$sourceAfter = (& $AdbPath -s $DeviceSerial shell $hashCommand) -join "`n"
if ($LASTEXITCODE -ne 0 -or $sourceAfter -ne $sourceBefore) {
    throw "The standard transmitter changed during installation."
}
...
if (-not $NoStart) {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Could not start the parallel transmitter." }
}

Write-Host "Standard transmitter hashes are unchanged."
Write-Host "Parallel transmitter installation completed."
