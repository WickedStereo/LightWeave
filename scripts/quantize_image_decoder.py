"""Quantize the fixed image decoder for QNN HTP with real latent samples."""
...
from lightweave.image import load_image_model, prepare_image  # noqa: E402
...
def quantize_decoder(
    ...
    for latent in values:
        ...
        max_absolute_errors.append(float(np.max(np.abs(expected - actual))))
    ...
    manifest.update(
        {
            ...
            "cpu_quantized_parity": {
                ...
                "maximum_absolute_error": max(max_absolute_errors),
...
def main() -> None:
    ...
    parser.add_argument(
        "--source", type=Path, default=artifact_dir / "image_decoder_fp32.onnx"
    )
    parser.add_argument(
        "--output", type=Path, default=artifact_dir / "image_decoder_qdq.onnx"
    )
    parser.add_argument(
        "--preprocessed",
        type=Path,
        default=artifact_dir / "image_decoder_preprocessed.onnx",
    )
    parser.add_argument(
        "--local-manifest",
        type=Path,
    ...
    parser.add_argument(
        ...
        default="uint16",
        help="QNN weight precision; uint16 preserves decoder fidelity better.",
    )
    args = parser.parse_args()
    image_paths = load_acceptance_images(args.demo_manifest, args.image_dir)
